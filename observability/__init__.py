...
def cluster_health_summary(
    ...
    # 2026-09-16：拿掉 container!="" 過濾條件——跟 prometheus_client.py 的
    # pod_cpu_usage()/pod_memory_usage_bytes() 是同一個問題：Docker Desktop 的
    # kubelet cAdvisor 只輸出 Pod 層級的 cgroup 彙總，沒有 container 這個 label
    # （不是空字串，是根本不存在），container!="" 會把僅有的資料排除掉，導致這台
    # 機器上查詢永遠回傳空結果。這個函式目前沒有任何呼叫端在用（純函式庫函式），
    # 但同一份 bug 已經在 prometheus_client.py 修過，這裡沒跟著改，之後有人接上時
    # 會重踩一次同樣的坑，一併修掉。
