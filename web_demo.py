...
import threading, json, urllib.request, urllib.parse, hashlib, secrets, re, uuid
import requests
...
# ── Google 登入（選用；沒設定 GOOGLE_CLIENT_ID/SECRET 時整個功能不出現，
#    不是顯示了讓使用者點下去才碰到錯誤——AGENT_RULES.md 新手友善原則）───────
...
def _derive_username_from_google(email: str) -> str:
    ...
    對方的 K8s namespace 存取權」，是帳號被冒用的風險（見 docs/security_review.md）。
    """
...
def _review_deployment(parsed, namespace=None):
    namespace = namespace or NS
    review = {
        "decision": "approve",
    ...
    # 2026-09-16 修正：這兩個檢查在多租戶 namespace 隔離上線後沒有跟著更新，一直查的是
    # 共用的 "default"（k8s_get_deployments()/k8s_get_services() 沒帶參數的預設值），
    # 跟 /api/deploy/conflicts（Chat 流程「下一步」用的，見 api_deploy_conflicts）已經
    # 修好的寫法不一致：名稱衝突要查「使用者自己的 namespace」（撞名才有意義），
    # port 衝突要跨所有 namespace 查（LoadBalancer port 綁定是 host 層級共用資源，
    # 見 docs/security_review.md 11 節）。
    try:
        app_name = parsed.get("app_name")
        if app_name and app_name in {d["name"] for d in k8s_get_deployments(namespace)}:
        ...
        if port:
            port = int(port)
            conflicts = [s for s in k8s_get_services(all_namespaces=True)
...
function renderSpecCard(flow){
  ...
  return flowShell(flow, `
    ...
    ${flow.hint ? `<div class="deploy-confirm-note">\ud83d\udca1 ${esc(flow.hint)}</div>` : ''}
    <div class="deploy-confirm-error" data-role="error"></div>
    <div class="deploy-confirm-actions">
...
async function flowExecuteDeploy(id){
  ...
  try{
    ...
    if(d.error || d.rejected){
      ...
      // \u6307\u4ee3\u6d88\u89e3\uff1a\u8a18\u4f4f\u9019\u500b\u804a\u5929\u5ba4\u6700\u8fd1\u4e00\u6b21\u6210\u529f\u52d5\u4f5c\u5230\u7684\u8cc7\u6e90\uff0c\u4e4b\u5f8c\u4f7f\u7528\u8005\u8aaa\u300c\u628a\u5b83
      // \u64f4\u5927\u5230 5 \u500b\u300d\u300c\u525b\u525b\u90e8\u7f72\u7684\u90a3\u500b\u300d\u624d\u6709\u6771\u897f\u53ef\u4ee5\u4ee3\u63db\uff08\u898b _resolve_intent_name\uff09\u3002
      const ch = currentChat();
      if(ch && flow.spec.app_name) ch.lastResource = {name: flow.spec.app_name, kind:'deployment'};
      loadStats && loadStats();
    }
  }catch(e){
    if(!flowAlive(flow)) return;
...
async function flowExecuteAction(id){
  ...
  try{
    ...
    if(ok){
      ...
      // healer_auto_fix \u4e00\u6b21\u8655\u7406\u4e00\u6279 Pod\uff0c\u6c92\u6709\u55ae\u4e00\u76ee\u6a19\u53ef\u4ee5\u8a18\uff1b\u5176\u9918\u52d5\u4f5c\u90fd\u6709\u660e\u78ba
      // \u7684\u55ae\u4e00\u8cc7\u6e90\uff0c\u8a18\u4e0b\u4f86\u7d66\u6307\u4ee3\u6d88\u89e3\u7528\uff08\u898b flowExecuteDeploy \u7684\u540c\u4e00\u500b\u6a5f\u5236\uff09\u3002
      if(flow.kind !== 'healer_auto_fix'){
        const ch = currentChat();
        const refName = a.name || a.pod_name;
        if(ch && refName) ch.lastResource = {name: refName, kind: flow.kind==='healer_fix' ? 'pod' : 'deployment'};
      }
      loadStats && loadStats();
    } else {
...
async function startDeployFlow(text){
  try{
    ...
    if(d.needs_clarification){
      // \u770b\u4e0d\u51fa\u4f7f\u7528\u8005\u60f3\u90e8\u7f72\u4ec0\u9ebc\u670d\u52d9\uff08\u9023 image \u7684\u7dda\u7d22\u90fd\u6c92\u6709\uff09\uff0c\u4e0d\u8981\u76f4\u63a5\u5957\u7528 nginx \u9810\u8a2d\u503c
      // \u9019\u7a2e\u5f4c\u88dc\u7684\u7b54\u6848\u9001\u9032 B1 \u5361\u7247\uff0c\u5148\u53cd\u554f\u6e05\u695a\u3002
      appendMsg('assistant', d.reason ||
        '\u770b\u4e0d\u51fa\u4f60\u60f3\u90e8\u7f72\u4ec0\u9ebc\u670d\u52d9\uff0c\u53ef\u4ee5\u8b1b\u6e05\u695a\u4e00\u9ede\u55ce\uff1f\u4f8b\u5982 image \u540d\u7a31\uff08nginx\u3001redis\u3001postgres...\uff09\u3002 / '+
        "I couldn't tell what service you want to deploy \u2014 could you specify, e.g. an image name like nginx, redis, or postgres?");
      return;
    }
    if(d.error || d.rejected){
    ...
    flow.resource = d.resource_summary || null;
    flow.hint = (d.parsed && d.parsed.hint) || null;
    appendMsg('assistant', renderFlowCard(flow));
  }catch(e){
...
function startClarify(intent){
  ...
  const label = {scale:'\u64f4\u7e2e\u526f\u672c',update_image:'\u66f4\u65b0\u6620\u50cf',rollback:'\u56de\u6efe',delete:'\u522a\u9664',
                 healer_fix:'\u4fee\u5fa9 Pod',healer_auto_fix:'\u81ea\u52d5\u4fee\u5fa9',
                 describe_pod:'\u67e5\u770b Pod',pod_health:'\u6aa2\u67e5\u5065\u5eb7\u72c0\u614b',
                 describe_deployment:'\u67e5\u770b\u90e8\u7f72'}[g] || g;
  const candidates = (intent.args && intent.args.candidates) || [];
  if(candidates.length){
    // \u6a21\u7cca\u6bd4\u5c0d\u6709\u5019\u9078\u4f46\u4e0d\u5920\u78ba\u5b9a\uff08\u5206\u6578\u4e0d\u5920\u9ad8\u6216\u524d\u5169\u540d\u592a\u63a5\u8fd1\uff09\uff0c\u4e0d\u81ea\u5df1\u9078\u4e00\u500b\uff0c
    // \u5217\u51fa\u5019\u9078\u8b93\u4f7f\u7528\u8005\u9078\uff0c\u907f\u514d\u7834\u58de\u6027\u64cd\u4f5c\u5957\u7528\u5230\u932f\u8aa4\u7684\u8cc7\u6e90\u3002
    const cid = 'clarify_' + (++_clarifySeq);
    _clarifyPending[cid] = Object.assign({}, intent.args);
    const btns = candidates.map(c =>
      `<button class="deploy-confirm-btn" onclick="resolveClarifyCandidate('${cid}','${g}','${c.name}')">${esc(c.name)}</button>`
    ).join(' ');
    appendMsg('assistant',
      `\u4f60\u662f\u6307\u4ee5\u4e0b\u54ea\u4e00\u500b\uff1f\uff08\u6a21\u7cca\u6bd4\u5c0d\u5230\u591a\u500b\u76f8\u4f3c\u540d\u7a31\uff0c\u4e0d\u5920\u78ba\u5b9a\u4e0d\u6703\u81ea\u52d5\u9078\uff09/ Did you mean one of these?<br>` + btns);
    return;
  }
  if(intent.args && intent.args.not_found){
    const name = intent.args.name || intent.args.pod_name || '';
    appendMsg('assistant',
      `\u627e\u4e0d\u5230\u53eb\u300c${esc(name)}\u300d\u7684 Pod \u6216 Deployment\uff0c\u53ef\u4ee5\u8f38\u5165 list pods / list deployments \u78ba\u8a8d\u6b63\u78ba\u540d\u7a31\u3002 / `+
      `Couldn't find anything named "${esc(name)}" \u2014 try "list pods" or "list deployments" to see the real names.`);
    return;
  }
  if(intent.args && intent.args.needs_reference){
    // \u4f7f\u7528\u8005\u8aaa\u300c\u5b83\u300d\u300c\u9019\u500b\u300d\u300c\u525b\u525b\u90e8\u7f72\u7684\u90a3\u500b\u300d\uff0c\u4f46\u9019\u500b\u804a\u5929\u5ba4\u9084\u6c92\u6709\u53ef\u4ee5\u53c3\u8003\u7684
    // \u4e0a\u4e00\u500b\u8cc7\u6e90\uff08\u4f8b\u5982\u65b0\u958b\u4e00\u500b Chat \u5c31\u76f4\u63a5\u9019\u6a23\u554f\uff09\u2014\u2014\u4e0d\u8981\u778e\u731c\uff0c\u76f4\u63a5\u53cd\u554f\u540d\u7a31\u3002
    appendMsg('assistant',
      `\u4f60\u662f\u6307\u54ea\u4e00\u500b\uff1f\u6211\u9019\u500b\u804a\u5929\u5ba4\u88e1\u9084\u6c92\u6709\u53ef\u4ee5\u53c3\u8003\u7684\u4e0a\u4e00\u500b\u8cc7\u6e90\uff0c\u8acb\u76f4\u63a5\u8b1b\u6e05\u695a\u540d\u7a31\u3002 / `+
      `Which one do you mean? I don't have a previous resource to refer to in this chat yet \u2014 please give a name.`);
    return;
  }
...
async function resolveClarifyCandidate(cid, action, chosenName){
  const args = Object.assign({}, _clarifyPending[cid] || {});
  delete _clarifyPending[cid];
  delete args.candidates; delete args.guess; delete args.not_found;
  const key = (action === 'healer_fix') ? 'pod_name' : 'name';
  args[key] = chosenName;
  appendMsg('user', chosenName);
  if(DESTRUCTIVE_KINDS.includes(action)){
    await startDestructiveFlow(action, args);
  } else if(READ_ACTIONS.includes(action)){
    window.__lastIntentArgs = args;
    appendTyping('Fetching');
    try{ await runReadAction(action); } finally { removeTyping(); }
  }
}
...
async function sendChat(){
  ...
  // 指代消解：跟著訊息一起把「這個聊天室最近一次提到的資源」傳給伺服器，
  // 使用者說「它」「這個」「剛剛部署的那個」時才有東西可以代換
  // （見 _resolve_intent_name，跟 flowExecuteDeploy/flowExecuteAction/runReadAction
  // 裡更新 lastResource 的地方是同一組機制）。
  const _chatForResource = currentChat();
  const _lastResourceName = (_chatForResource && _chatForResource.lastResource && _chatForResource.lastResource.name) || null;

  let intent = matchClientRule(text);
  const _needsNameCheck = intent && (intent.args.name || intent.args.pod_name) &&
    ['delete','scale','update_image','rollback','healer_fix','describe_pod','pod_health','describe_deployment'].includes(intent.action);
  if(intent && _needsNameCheck){
    // 前端規則只是字面上抓到一個名字（可能是打錯字，也可能是「它」這種指代詞），
    // 不代表這個字串在叢集裡真的存在——交給伺服器核對一次，核對不過會降級成 clarify。
    try{
      const r = await fetch('/api/intent',{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({message:text, client_intent:intent, last_resource:_lastResourceName})});
      intent = await r.json();
    }catch(e){ /* 核對本身失敗就沿用前端解析結果，不要因為網路問題卡住 */ }
  } else if(!intent){
    try{
      const r = await fetch('/api/intent',{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({message:text, last_resource:_lastResourceName})});
      intent = await r.json();
...
def index():
    ...
    return render_template_string(HTML, logged_in=True, page='app', error=None, k8s=K8S_ENABLED, username=session["username"], google_login_available=_google_oauth_configured())
...
# ── 模糊比對真實資源名稱：不管動作是規則層還是模型分類出來的，指名資源前都要 ──
# 核對這個名字在叢集裡到底存不存在／夠不夠確定，核對不過就轉成 clarify（可能
# 附候選清單）。deploy 不在這個集合裡——它的 app_name 是「要建立的新名稱」，
# 不是要在既有資源裡找一個。
_NAME_ARG_ACTIONS = {
    "describe_pod", "pod_health", "describe_deployment",
    "scale", "update_image", "rollback", "delete", "healer_fix",
}
_NAME_ARG_KEY = {"healer_fix": "pod_name"}  # 其餘動作的名稱欄位都叫 "name"


def _all_resource_names(namespace: str) -> list:
    """給模糊比對用的候選名單：這個帳號自己 namespace 底下所有 pod + deployment 名稱。
    查不到（K8s 未連線等）回空清單，呼叫端要把空清單當成「沒有候選名單可以核對」，
    不能因此擋住整個查詢/操作。"""
    try:
        names = {p["name"] for p in k8s_get_pods(namespace=namespace)}
        names |= {d["name"] for d in k8s_get_deployments(namespace)}
        return sorted(names)
    except Exception:
        return []


# ── 指代消解：matchClientRule()/_rule_intent() 既有的正則（例如「把X擴大到N個」的
# (\S+?) 非貪婪擷取）在使用者說「把它擴大到 5 個」「剛剛部署的那個回滾」這種話時，
# 會原封不動把指代詞本身（它/這個/剛剛部署的那個...）擷取成 name——不是抓不到名稱，
# 是抓到的字串根本不是名稱。這裡只核對「已經被擷取出來的 name 字串」是不是指代詞，
# 不對整句訊息做全文替換，範圍限縮在既有機制已經確定「這是要填進 name 欄位的值」
# 之後，不會誤觸發在無關的一般問答句子裡（那些句子從一開始就不會被歸類成需要
# name 的動作）。
_PRONOUN_RE = re.compile(
    r'^(?:它|它們|這個|那個|這一個|那一個|'
    r'剛剛(?:部署|建立|弄|起|做)?的?(?:那個|那些)?|'
    r'剛才(?:部署|建立|弄|起|做)?的?(?:那個|那些)?|'
    r'it|this(?:\s+one)?|that(?:\s+one)?)$',
    re.IGNORECASE,
)


def _looks_like_pronoun(name: str) -> bool:
    """判斷一個被規則/模型抓出來、原本要當「資源名稱」用的字串，是不是其實只是
    指代詞（它/這個/那個/剛剛部署的那個/it/that one...），不是真的資源名稱。"""
    return bool(_PRONOUN_RE.match((name or "").strip()))


def _resolve_intent_name(intent: dict, namespace: str, last_resource: str = None) -> dict:
    """對「需要指名資源」的意圖多做一層核對：
      1. 抓到的名稱其實是指代詞（它/這個/剛剛部署的那個...）→ 換成 last_resource
         （聊天室記得的「上一個提到的資源」，由前端跟著訊息一起傳來）；沒有
         last_resource 可用就直接反問「你是指哪一個」，不要瞎猜。
      2. 模糊比對：
         - 夠確定（完全相符/前綴/高分打字錯誤，且跟第二名分數拉開）→ 直接把
           args 裡的名稱換成核對後的真實名稱，靜默修正打字錯誤，不用另外反問
         - 有候選但不夠確定（例如「快取」比對到多個服務、或兩個名稱分數太接近）
           → 降級成 clarify，附上候選清單給前端做快速選擇
         - 完全沒有候選 → 降級成 clarify，講清楚「找不到這個名字」，不要讓它
           繼續往下跑到真正呼叫 K8s API 才用一個生硬的錯誤訊息失敗
    查不到叢集資源清單時（namespace 底下沒有任何 pod/deployment，或 K8s 未連線）
    原樣放行，不要因為核對不了就擋住功能——這只是錦上添花的安全網，不是唯一防線。
    """
    action = intent.get("action")
    if action not in _NAME_ARG_ACTIONS:
        return intent
    key = _NAME_ARG_KEY.get(action, "name")
    raw_name = (intent.get("args") or {}).get(key)
    if not raw_name:
        return intent

    if _looks_like_pronoun(raw_name):
        if last_resource:
            intent = dict(intent, args=dict(intent.get("args") or {}, **{key: last_resource}))
            raw_name = last_resource
        else:
            return {
                "action": "clarify",
                "args": {"guess": action, "needs_reference": True},
                "confidence": intent.get("confidence", 0.0),
            }

    candidates_pool = _all_resource_names(namespace)
    if not candidates_pool:
        return intent
    from core.fuzzy_match import fuzzy_match_resource, pick_confident_match
    matches = fuzzy_match_resource(raw_name, candidates_pool)
    confident = pick_confident_match(matches)
    if confident:
        if confident["name"] != raw_name:
            intent = dict(intent, args=dict(intent.get("args") or {}, **{key: confident["name"]}))
        return intent
    if matches:
        return {
            "action": "clarify",
            "args": {"guess": action, key: raw_name, "candidates": matches[:5]},
            "confidence": intent.get("confidence", 0.0),
        }
    return {
        "action": "clarify",
        "args": {"guess": action, key: raw_name, "not_found": True},
        "confidence": intent.get("confidence", 0.0),
    }
...
def api_intent():
    ...
    matchClientRule 命中但動作需要指名資源時，也會帶 client_intent 打這裡，
    只為了做模糊比對核對（見 _resolve_intent_name），不重新分類意圖。
    回 {action, args, confidence, source}。source: rule | llm | llm_unavailable。
    action 可能是 clarify（模型不確定的破壞性操作、指名的資源模糊或找不到）或
    qa（純問答，交回 /api/chat）。"""
    if "username" not in session:
        return jsonify({"error": "Not authenticated"}), 401
    data = request.get_json() or {}
    user_ns = _user_namespace(session["username"])
    # 前端跟著訊息一起傳「這個聊天室最近一次提到的資源名稱」，給指代消解用
    # （見 _resolve_intent_name 對 it/這個/那個/剛剛部署的那個 這類指代詞的處理）。
    last_resource = (data.get("last_resource") or "").strip() or None

    client_intent = data.get("client_intent")
    if isinstance(client_intent, dict) and client_intent.get("action"):
        result = _resolve_intent_name(
            {"action": client_intent.get("action"), "args": client_intent.get("args") or {},
             "confidence": 1.0},
            user_ns, last_resource,
        )
    ...
    if hit:
        result = {**hit, "confidence": 1.0, "source": "rule"}
    else:
        try:
    ...
    result = _resolve_intent_name(result, user_ns, last_resource)
...
def _prepare_deploy(user_input: str, parsed_override: dict = None, namespace: str = None):
    ...
    # 部署缺欄位時，猜還是反問：parsed_override 是使用者自己在確認卡片上編輯過、
    # 按下一步送回來的值，不需要再反問一次（_sanitize_deploy_payload 也不會帶
    # _ambiguous 這個 key，這個分支只會在真正從 ask_llama() 現生的 spec 上觸發）。
    if not parsed_override and parsed.get("_ambiguous"):
        return parsed, enriched, None, ({
            "parsed": enriched,
            "needs_clarification": True,
            "reason": parsed.get("_ambiguous_reason", "看不出想部署什麼服務"),
        }, 200)
    ...
    review = _review_deployment(parsed, namespace=namespace)
    try:
...
def api_deploy_parse():
    ...
    parsed, enriched, review, error = _prepare_deploy(user_input, parsed_override, namespace=user_ns)
    if error:
