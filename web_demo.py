...
import threading, json, urllib.request, hashlib, secrets
...
async function doDeploy(){
  ...
  try {
    ...
    if(d.error){
      ...
      const p = d.parsed;
      box.textContent = `✓ Deployed successfully!\n\nApp: ${p.app_name}\nImage: ${p.image}\nPods: ${p.pods}${p.port ? '\nPort: ' + p.port : ''}${p.memory ? '\nMemory: ' + p.memory : ''}\n\nK8s: ${d.k8s ? 'Real deployment' : 'Simulation mode'}`;
      loadStats();
    }
  } catch(e){
    box.style.display = 'block';
