...
def main():
    ...
    try:
        ...
        # write_manifest() 實際回傳的 key 是 ok/files/commit_sha/message（不是
        # committed/commit_hash/manifest_path，先前這裡打錯 key 名稱，導致這兩個分支
        # 都拿不到值、永遠印出「YAML 已寫入：」後面空白——commit 動作本身沒有受影響，
        # 只是這則確認訊息一直是錯的。
        gitops_result = write_manifest(final_json, repo_path=".", dry_run=False, commit=True)
        if gitops_result.get("commit_sha"):
            print(f"[GitOps] 已 commit：{gitops_result['commit_sha'][:8]}")
        elif gitops_result.get("files"):
            print(f"[GitOps] YAML 已寫入：{gitops_result['files'][0]}")
        else:
            print(f"[GitOps] {gitops_result.get('message', '完成')}")
