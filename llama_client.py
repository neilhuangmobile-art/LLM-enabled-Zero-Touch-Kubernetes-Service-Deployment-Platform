...
def _finalize_deploy_result(raw_input: str, result: dict) -> dict:
    """ask_llama() 每一個成功路徑（Gemini 快路徑/deterministic/Claude/3B/本地 fallback）
    回傳結果前都要經過這裡：判斷這份 spec 是不是用預設值蒙混過去，而不是真的問出
    使用者要什麼。標記進底線開頭的 metadata key（沿用既有 `_parser` 這種慣例，
    不影響下游讀取 pods/image/app_name 等既有欄位）。core/deploy_ambiguity.py
    獨立成不需要 torch 的小模組，這裡延遲 import 沒有實質差異（llama_client.py
    本來就無條件 import torch），純粹是模組職責分離。"""
    if not isinstance(result, dict) or "error" in result:
        return result
    try:
        from core.deploy_ambiguity import deploy_ambiguity_check
        check = deploy_ambiguity_check(raw_input, result)
        if check.get("ambiguous"):
            result["_ambiguous"] = True
            result["_ambiguous_reason"] = check.get("reason", "")
        if check.get("hint"):
            result["_hint"] = check["hint"]
    except Exception:
        pass
    return result
...
def ask_llama(prompt_text: str) -> dict:
    ...
    try:
                ...
                if gemini_available():
                    ...
                    if spec_block:
                        ...
                        if spec and _validate(spec):
                            return _finalize_deploy_result(prompt_text, spec)
        ...
        if deterministic and _validate(deterministic):
            ...
            return _finalize_deploy_result(prompt_text, deterministic)
            ...
            try:
                ...
                if claude_available():
                    ...
                    if result is not None:
                        ...
                        return _finalize_deploy_result(prompt_text, result)
            except Exception:
                pass
        ...
        if server_result is not None:
            return _finalize_deploy_result(prompt_text, server_result)

        # ── 5. Fallback：本地直接載入 ──
        return _finalize_deploy_result(prompt_text, _local_infer(model_input))
