...
**已知殘留限制**：這是黑名單式的確定性規則，只能擋「已知手法」，不是萬無一失——
換一種還沒想到的措辭大概率繞得過去。跟 `agents/guardian` 的三層防護（orchestrator →
guardian → gitops）比，那三層是「不管 LLM 怎麼被騙，最後產出的 YAML 還是會被獨立驗證」，
這裡的 `/chat` 沒有等價的「輸出層防線」——因為 `/chat` 純粹是問答，沒有 YAML 可以驗證。
`/chat` 的回覆目前也**不會**被拿去驅動任何 K8s 操作（部署/scale/delete 走 `_rule_intent`
＋`/api/intent` 分類＋多步確認卡這條完全獨立的路徑，不吃 `/chat` 的自由文字輸出），所以
這個漏洞目前的實際危害範圍是「使用者可能被自己送出的訊息誤導、看到錯的聊天回覆」，
不是「攻擊者能透過 Chat 觸發未授權的叢集操作」——但如果之後有計劃讓 `/chat` 的輸出
更直接驅動行為，這個界線要重新評估。
...
- `_looks_like_prompt_injection()` 是黑名單規則，只能擋已知手法，之後如果發現新的繞過
  措辭要持續補規則（見 9 節「已知殘留限制」）；`/diagnose` 端點雖然也套用了同一套文字
  淨化，但沒有等價的確定性攔截規則擋 pod 日誌裡的注入文字，優先度較低（因為背景自動修復
  的動作集合本身就有限，見 6 節），但屬於同一類風險，值得之後補齊。
