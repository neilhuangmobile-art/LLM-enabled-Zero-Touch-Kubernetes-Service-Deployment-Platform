...
## 13. Google 登入（Sign in with Google）：新增第二種登入方式，跟既有帳密系統共存

**風險**：新增 OAuth 登入這類功能，常見的實作陷阱是（1）沒驗證 OAuth 的 `state`
參數，讓攻擊者能用偽造的回呼連結誘騙使用者登入攻擊者控制的帳號（session
fixation/CSRF 的一種變形）；（2）沒檢查 email 是否已驗證，讓任何人只要能操控
OAuth 流程的某個環節就能宣稱擁有別人的 email；（3）用「email 前綴」或「email
本身」當作跟既有帳號比對/合併的依據，等於只要知道對方的 username 或 email，
就能用自己的 Google 帳號登入拿到對方的身分（含 K8s namespace 存取權）。

**為什麼重要**：這個平台每個帳號對應一個獨立的 K8s namespace（見 11 節），身分
系統的正確性直接等於資料隔離的正確性——認證機制被繞過或帳號被冒用，等於直接
繞過整個多租戶隔離設計。新增登入方式的當下沒做對，後面所有基於 `session["username"]`
的授權判斷（`_enforce_ban`、`_user_namespace`、每個 API 路由的登入檢查）都會連帶
被架空。

**解決方式**（`web_demo.py` 新增 `_google_oauth_configured()`/
`_derive_username_from_google()`/`google_login`/`google_callback`）：
...
- **絕不自動合併/連結帳號**：`_derive_username_from_google()` 用 email 前綴衍生
  username，但**只要跟任何既有帳號撞名（不管是密碼帳號還是別的 Google 帳號）就一律
  加 hash 後綴變成新帳號**，不會覆蓋、也不會嘗試「猜測」這是不是同一個人。查找
  既有 Google 帳號的依據是 `google_sub`（Google 的穩定內部 ID，不會變動），不是
  email 或 username 字串比對——確保「這是同一個 Google 帳號第二次登入」的判斷
  完全基於 Google 自己保證唯一且不變的識別碼，不是任何可能被猜到或巧合撞到的值。
- **共存不取代**：純密碼帳號完全不受影響；`login()` 遇到「這個帳號是純 Google
  帳號、沒有 `password_hash`」的情況，會給出「請改用 Google 登入」的明確訊息，
  不是含糊的「帳號密碼錯誤」（AGENT_RULES.md 新手友善原則）。
- **新手友善的降級**：`GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` 沒設定時，登入/
  註冊頁的按鈕整個不出現；就算使用者直接打 `/auth/google/login` 網址，也是回一個
  講清楚原因的頁面，不是 404/500。
- 沒有把 Google 的 `access_token`/`refresh_token` 存下來——只在 callback 那次請求
  裡短暫使用（換使用者資訊），用完即棄，減少憑證外洩時的影響範圍（反正這個功能
  只拿 Google 帳號當身分核驗，不需要之後代表使用者呼叫其他 Google API）。

**驗證**：新增 `tests/test_google_auth.py`（18 案例，monkeypatch 掉真正打 Google
...
帳號時正確加後綴而非合併、按鈕在設定/未設定兩種情況下正確顯示/隱藏。跑過既有
`pytest`（203 個）全過，確認純密碼登入路徑沒有回歸。手動驗證：實際申請 Google
OAuth 憑證、跑一次真實登入流程，確認 `users.json` 正確寫入、K8s namespace 正確建立
——這部分需要使用者自己的 Google Cloud 帳號才能做，留給使用者接續驗證。

---

## 14. 部署前只擋「單一 Pod 太大」，沒擋「疊加其他部署後總量超出容量」
...
**解決方式**：新增共用邏輯 `_sum_cluster_resource_requests()`（加總全叢集所有
namespace 目前所有 Deployment 的真實資源需求，用真實 replicas 數，不是理論值）；
`_prepare_deploy()` 在既有的「單一 Pod 過大」檢查之後，新增第二層：現有叢集總量
+ 這次請求的 cpu/mem × 部署的副本數，超出叢集最大節點容量就直接 `block`（跟單一
Pod 過大同一個嚴重等級，不是只給警告放行），訊息講清楚為什麼會這樣、多出來的
Pod 會怎樣、以及具體該怎麼降低需求或先清空間。既有的 `_check_scale_risk()`（事後
調整 replicas 用）維持原本已驗證過的內嵌加總寫法不變，新函式只服務這個新的
初次部署檢查路徑，避免無謂改動已通過測試的既有函式。

**驗證**：直接呼叫 `_prepare_deploy()` 測試「單一 Pod 放得下（15 核），但要求
2 個副本，疊加現有叢集用量後總計 30.5 核 > 20 核節點容量」的案例，確認正確回傳
`rejected: true` 並附上具體數字；對照組（200m CPU / 256Mi 的正常規模部署）確認
沒有被誤擋，只收到跟容量無關的其他既有警告（latest tag、缺 readinessProbe 等）。
這項功能由組員在 `feat/chat-unified-assistant` 分支開發、2026-09-19 移植進本機
`feat/chat-unified-assistant-local` 分支，移植後 203 個測試全過。
